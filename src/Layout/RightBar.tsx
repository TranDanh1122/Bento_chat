        ...
        <aside className=" w-3/12 h-full min-h-full bg-[#000]/8 p-3 flex flex-col gap-3">
            ...
            {viewing == "follow" &&
                <div className="flex flex-col gap-2">
                    <div className="bg-[#f8f8f8]/10 rounded-xl p-3">
                        <User user={{} as User}>
                            <img src="/images/add.svg" alt="" className="w-10 h-10 object-cover ml-auto cursor-pointer" />
                        </User>
                        <span className="body_14_r text-opacity-80 text-[#f8f8f8]/50 block text-center mt-3">I design digital products and ventures.</span>
                    </div>
                    <div className="bg-[#f8f8f8]/10 rounded-xl p-3">
                        <User user={{} as User}>
                    ...
                    <div className="flex items-center justify-center relative">
                        ...
                        <span className="base_14_sm text-[#f8f8f8]/70 translate-x-[-10%] bg-black/70 rounded-3xl relative z-10 py-[6px] px-3">1,234+</span>
                    </div>
                </div>}
            {viewing == "topics" &&
                <div className="flex flex-col gap-2">
                    <div className="flex item-centers gap-4 bg-[#f8f8f8]/10 rounded-xl">
                        <img src="/images/topic.png" alt="" className="max-w-28  object-cover" />
                        <div className="flex flex-wrap gap-y-1 gap-2 p-4">
                            <h2 className="line-clamp-2 base_14_sm text-[#f8f8f8]/95 text-opacity-80">Apple Releases iOS 17.6.1 with Advanced Data Protection Fix</h2>
                            <img src="/images/avt_group.png" alt="" className="w-14 h-5 object-cover" />
                            <span className="caption_r text-[#f8f8f8]/50 text-opacity-45">4 hours ago</span>
                        </div>
                    </div>
                    {topics?.map(topic => <div className="flex item-centers gap-4 bg-[#f8f8f8]/10 rounded-xl">
                        <img src="/images/topic.png" alt="" className="max-w-28  object-cover" />
                        <div className="flex flex-wrap gap-y-1 gap-2 p-4">
                            <h2 className="line-clamp-2 base_14_sm text-[#f8f8f8]/95 text-opacity-80">Apple Releases iOS 17.6.1 with Advanced Data Protection Fix</h2>
                            <img src="/images/avt_group.png" alt="" className="w-14 h-5 object-cover" />
